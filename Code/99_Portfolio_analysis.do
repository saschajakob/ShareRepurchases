...
*	use "$data2/AAA_index_vw_100m.dta", clear
	use "$data2/AAA_index_vw_sp500.dta", clear
	
	* Collapse the data to the portfolio level
	collapse pf_ret36m n_obs36m pf_ret36m_tc pf_ret36m_tc_tic pf_ret36m_tc_tic_val rf mktrf smb hml umd,  by(month)
...
*	use "$data2/AAA_index_ew_100m.dta", clear	
	use "$data2/AAA_index_ew_sp500.dta", clear	
	...
	* Load data 
	...
	merge 1:1 month using index_returns2.dta
	keep if _merge == 3
	drop _merge	
...
*/	
	...
	* Plot portfolio performance using log scale
	...
	line ri_vw rirus3000 month, yscale(log)
	line ri_ew ri_vw rirus3000 month, yscale(log)
	
	line ri_ew risp500 month, yscale(log)
	line ri_vw risp500 month, yscale(log)
	line ri_ew ri_vw risp500 month, yscale(log)	
