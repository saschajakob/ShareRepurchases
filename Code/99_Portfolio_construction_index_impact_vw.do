...
*/	
	...
	*gen index = cash_rank + ivol_rank + ret_rank + payout_rank if mkvalt > 100000
...
*/	
	* Compute returns and number of observations for overall portfolio
	egen totmk = total(mkvalt) if index >=16 & dist == 1 & insample == 1 & index != .
	g weight = mkvalt/totmk
	g temp = weight*ret
	*egen temp2 = sum(temp), by(month) 
	
	*egen temp = mean(ret) if dist == 1 & insample == 1
	
	* Compute the number of observations for portfolio	
			...
			*rebalancing costs
			...
			gen tc = tw_diff *0.0020																					// assuming transaction costs of 20 basis points
				...
				*trading impact
				...
				gen wic = weight*ic
				egen tic = total(wic) if dist==1, by(month)
	...
	* Drop all variables of look and rerun three months later	
	drop dist tag tag2 tag3 q_cash q_profit q_ret q_ivol cash_rank profit_rank ret_rank ivol_rank index insample temp temp2 temp3 temp4 temp5 temp6 obs1 obs2 obs3 obs4 weight t1_weight t2_weight mtemp1-mtemp6 totmk totmk2 ic wic tic tc tw_diff w_diff
		
	}
	
	drop quarter_w_diff_top quarter_w4_top quarter_w_diff_bottom quarter_w4_bottom quarter_w_diff quarter_w4
	
	save "$temp/AAA_index_vw_sp500.dta", replace
	save "$data2/AAA_index_vw_sp500.dta", replace	
	****************************************************************************
