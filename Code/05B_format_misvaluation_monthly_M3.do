...
global data		"/Users/saschajakob/Desktop/Paper 1"
...
capture program drop _all
	...
	********************************************
	use "$data/CRSP_monthly_1970_2019.dta", clear
	...
	*/  using "$data/CRSP_Comp_merged_1950_2018.dta", clear
...
*	keep if (fyear<=2010)
	...
	*sum year_id, det
	...
	forval i = 1(1)`r(max)' {
		...
		qui capture reg ln_mkvalt ln_bkvalt_l1 ln_ni I_ni b_lev  if month_id == `i' & ff12 == `j'
		if _rc != 0{
		continue
		}
		...
		replace alpha2_jt = _b[ln_ni] if month_id == `i' & ff12 == `j'
		replace alpha3_jt = _b[I_ni] if month_id == `i' & ff12 == `j'
		replace alpha4_jt = _b[b_lev] if month_id == `i' & ff12 == `j'
	...
	* We take the time-series average of the alphas for every industry
	...
	egen alpha_bar2_j = mean(alpha2_jt), by(ff12)
	egen alpha_bar3_j = mean(alpha3_jt), by(ff12)
	egen alpha_bar4_j = mean(alpha4_jt), by(ff12)


	* Generate fundamental value variables
	g v_alpha_jt = alpha0_jt + alpha1_jt * ln_bkvalt_l1 + alpha2_jt * ln_ni + alpha3_jt * I_ni + alpha4_jt * b_lev
	g v_alpha_bar_j = alpha_bar0_j + alpha_bar1_j * ln_bkvalt_l1 + alpha_bar2_j * ln_ni + alpha_bar3_j * I_ni + alpha_bar4_j * b_lev
	...
	* Generate misvaluation variables
	...
	keep permno  month ln_mkvalt ln_bkvalt ln_bkvalt_l1 mk_bk alpha0_jt alpha1_jt alpha2_jt alpha3_jt alpha4_jt alpha_bar0_j alpha_bar1_j alpha_bar2_j alpha_bar3_j alpha_bar4_j v_alpha_jt v_alpha_bar_j RKRV_firm_error RKRV_sector_error RKRV_longrun_vb

	sort permno month

	save "$data/RKRV_misvaluation_monthly_3M.dta", replace
