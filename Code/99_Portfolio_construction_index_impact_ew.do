...
capture program drop _all
	...
	*************************************************************		
	...
	use "$data2/crsp_sdc_comp_clean.dta", replace		
	...
	* Summarize the four sorting variables
	...
	tabstat cash_atl1 ivol grossprofit_atl1  six_m_cum_ret, stats(n mean sd p50 min p25 p75 max) c(s)
...
*	drop if payout_yield == .
	...
	* December 2019 is time2 = 719	
	* December 1994 is time2 = 419
	g pf_time = time2 - 419
/*
	g pf_ret12m = .
	g top_pf_ret12m = .
...
*	g ret_sd = 0.3
	...
	* quarter transition weight variable
	...
	replace pf_ret36m_tc_tic_val = 500000 if time2 ==419													// chose initial portfolio size
...
*	egen q_payout = xtile(payout_yield_all) if dist==0 & tag3 == 1 & mkvalt > 100000, nq(5)	
	...
	* Create the index - Add a constraint of minimum market cap of 100m
*	egen q_cash = xtile(cash_atl1) if dist==0 & tag3 == 1 & sp500stock == 1, nq(5)		
*	egen q_ivol = xtile(ivol) if dist==0 & tag3 == 1 & sp500stock == 1, nq(5) 		
*	egen q_owner = xtile(top10instown_percl1) if dist==0 & tag3 == 1, nq(5)	
*	egen q_ret = xtile(six_m_cum_ret) if dist == 0 & tag3 == 1 & sp500stock == 1, nq(5) 	
*	egen q_payout = xtile(payout_yield_all) if dist==0 & tag3 == 1 & mkvalt > 100000, nq(5)	
*	egen q_profit = xtile(grossprofit_atl1) if dist==0 & tag3 == 1 & sp500stock == 1, nq(5)
...
*/	
	...
	*gen index = cash_rank + profit_rank + ret_rank + ivol_rank
	gen index = cash_rank + profit_rank + ret_rank + ivol_rank	if mkvalt > 100000
...
*/	
	* Compute returns and number of observations for overall portfolio
	...
	egen temp = mean(ret)  if index >=16 & dist == 1 & insample == 1 & index != .
	egen obs1 = count(ret) if index >=16 & dist == 1 & insample == 1 & index != .
	egen obs2 = count(ret) if index >=16 & dist == 2 & insample == 1 & index != .
	egen obs3 = count(ret) if index >=16 & dist == 3 & insample == 1 & index != .
	egen obs4 = count(ret) if index >=16 & dist == 4 & insample == 1 & index != .

	* Tag the firms that are in the portfolio in a given months
	replace pf_firm = 1 if index >=16 & dist == 1 & insample == 1 & index != .
	replace pf_firm = 1 if index >=16 & dist == 2 & insample == 1 & index != .
	replace pf_firm = 1 if index >=16 & dist == 3 & insample == 1 & index != .	
	
	* Compute average and median market cap
	egen mtemp1 = mean(mkvalt) if index >=16 & dist == 1 & insample == 1 & index != ., by(month)
	egen mtemp2 = mean(mkvalt) if index >=16 & dist == 2 & insample == 1 & index != ., by(month)
	egen mtemp3 = mean(mkvalt) if index >=16 & dist == 3 & insample == 1 & index != ., by(month)
	
	replace mkvalt36m = mtemp1 if dist == 1
	replace mkvalt36m = mtemp2 if dist == 2
	replace mkvalt36m = mtemp3 if dist == 3		
	*
	egen mtemp4 = median(mkvalt) if index >=16 & dist == 1 & insample == 1 & index != ., by(month)
	egen mtemp5 = median(mkvalt) if index >=16 & dist == 2 & insample == 1 & index != ., by(month)
	egen mtemp6 = median(mkvalt) if index >=16 & dist == 3 & insample == 1 & index != ., by(month)
	...
	* Drop all variables of look and rerun three months later	
	drop dist tag tag2 tag3 q_cash q_profit q_ret q_ivol cash_rank profit_rank ret_rank ivol_rank index insample temp temp2 temp3 temp4 temp5 temp6 obs1 obs2 obs3 obs4 weight t1_weight t2_weight tc tw_diff w_diff mtemp1-mtemp6 ic tic
		
	}
	
	drop quarter_w_diff_top quarter_w4_top quarter_w_diff_bottom quarter_w4_bottom quarter_w_diff quarter_w4
	
	save "$temp/AAA_index_ew_100m.dta", replace
	save "$data2/AAA_index_ew_100m.dta", replace	
	****************************************************************************
