...
with warnings.catch_warnings():
    ...
    for permno, key in tqdm(groups):
        try:
            if key.shape[0]<24:                                                                       # define minimum number of observations
                raise ValueError('Too few observations')
            model_full = VAR(key[['lq_ret','lbtm_q','lroe_q']])
            results_full = model_full.fit(1)
            resid_mat = pd.DataFrame(np.matrix(results_full.resid))
            coeff_mat = np.matrix(results_full.coefs)
            sigma_mat = np.matrix(results_full.sigma_u)
            key = key[1:]
            key = key.reset_index()
            key = key.join(resid_mat)
            key.set_index('ccm_yq', inplace=True)
            for index, row in key.iterrows():
                if row['rep_q']==1:
                    try:
                        idx = index
                        pre_event_df = key.loc[idx - 20 : idx - 1]
                        pre_resid_mat = np.matrix(resid_mat.iloc[0:20,-3:])
                        pre_sigma_mat = np.matrix(resid_mat.iloc[0:20,-3:].cov())
                        pre_event_df = pre_event_df.iloc[:,0:-3]
                        ...
                        post_resid_mat = np.matrix(resid_mat.iloc[21:,-3:])
                        post_sigma_mat = np.matrix(resid_mat.iloc[21:,-3:].cov())
                        post_event_df = post_event_df.iloc[:,0:-3]
                        if post_event_df.shape[0]<12:                                                   # define minimum number of observations subsequent to the repurchase announcement
                            raise ValueError('Too few observations')

                        # decomposition
                        lambda_full = np.matrix(e1 @ (discount_f*coeff_mat) @ np.linalg.inv(ID_mat -discount_f*coeff_mat)).T

                        # discount-rate
                        # level
                        pre_disc_lvl = np.mean(pre_resid_mat.dot(lambda_full))
                        post_disc_lvl = np.mean(post_resid_mat.dot(lambda_full))

                        # variance
                        pre_disc_var = (lambda_full.T @ pre_sigma_mat @ lambda_full)[0].item()
                        post_disc_var = (lambda_full.T @ post_sigma_mat @ lambda_full)[0].item()

                        # cash-flow
                        # level
                        pre_cf_lvl = np.mean(pre_resid_mat.dot(e1 + lambda_full))
                        post_cf_lvl = np.mean(post_resid_mat.dot(e1 + lambda_full))

                        # variance
                        pre_cf_var = ((e1 + lambda_full.T) @ pre_sigma_mat @ (e1 + lambda_full.T).T)[0].item()
                        post_cf_var = ((e1 + lambda_full.T) @ post_sigma_mat @ (e1 + lambda_full.T).T)[0].item()
...
decomp_df.to_stata('A:\\Sascha\\Buyback Anomalies\\decomp_data_single_sample.dta')
...
model_full = VAR(subset_df[['lq_ret','lbtm_q','lroe_q']])
results_full = model_full.fit(1)
resid_mat = pd.DataFrame( np.matrix(results_full.resid))
coeff_mat = np.matrix(results_full.coefs)
sigma_mat = np.matrix(results_full.sigma_u)
...
subset_df = subset_df.join(resid_mat)
subset_df.set_index('ccm_yq', inplace=True)

idx = subset_df.index.values[27]
idx
pre_event_df = subset_df.loc[idx - 20: idx - 1]
pre_sigma_mat = np.matrix(pre_event_df.iloc[:,-3:].cov())
pre_sigma_mat
pre_resid_mat = np.matrix(pre_event_df.iloc[:,-3:])
pre_resid_mat
pre_event_df = pre_event_df.iloc[:,0:-3]
pre_event_df
