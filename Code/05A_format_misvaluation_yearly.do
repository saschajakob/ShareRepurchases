...
*  	g calmonth	= month(date)
	...
	* These alphas are estimated for every industry-year
	...
	forval i = 1(1)`r(max)' {
		...
		qui capture reg ln_mkvalt ln_bkvalt if year_id == `i' & ff12 == `j'
		if _rc != 0{
		continue
		}

		replace alpha0_jt = _b[_cons] if year_id == `i' & ff12 == `j'
		replace alpha1_jt = _b[ln_bkvalt] if year_id == `i' & ff12 == `j'
	...
	* Generate one year lag
	...
	keep gvkey permno fyear ln_mkvalt ln_bkvalt RKRV_firm_error RKRV_sector_error RKRV_longrun_vb `vlistl1'


	sort permno fyear

	save "$data/RKRV_misvaluation_yearly.dta", replace
