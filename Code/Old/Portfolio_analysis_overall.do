...
*	use "$data2/pf_mid_crsp.dta"	
	use "$data2/pf_small_crsp.dta"	
	
	
	* Collapse the data to the portfolio level
	collapse pfret_ew pfret_vw pf_obs pf_mkvalt pf_mkvalt_med rf mktrf smb hml umd cma rmw ps_vwf,  by(month)
	...
	save "$temp/ret_all_crsp.dta", replace
	
	
	* Load data 
	use "$temp/ret_all_crsp.dta", clear
