...
*	use "$data2/sample_returns_all_36m_cashretprofitivol.dta"
	...
	* Load data for mega stocks
	use "$data2/sample_36m_mega_vw.dta"
	...
	* Collapse the data to the portfolio level
	...
	save "$temp/ret_mega_vw.dta", replace
	
	* Load data for large stocks
	use "$data2/sample_36m_large_vw.dta", clear
	...
	* Collapse the data to the portfolio level
	...
	save "$temp/ret_large_vw.dta", replace	
	
	* Load data for mid stocks
	use "$data2/sample_36m_mid_vw.dta", clear
	...
	* Collapse the data to the portfolio level
	...
	save "$temp/ret_mid_vw.dta", replace		
	
	* Load data for small stocks
	use "$data2/sample_36m_small_vw.dta", clear
	...
	* Collapse the data to the portfolio level
	...
	save "$temp/ret_small_vw.dta", replace			
	
	* Load data for overall sample stocks
	use "$data2/sample_36m_full_vw.dta", clear
	...
	* Collapse the data to the portfolio level
	...
	save "$temp/ret_full_vw.dta", replace			
/*	
	* Load data for overall sample stocks
...
*/	
	use "$temp/ret_full_vw.dta", clear	

	merge 1:1 month using "$temp/ret_mega_vw.dta"
	drop _merge
	
	merge 1:1 month using "$temp/ret_large_vw.dta"
	drop _merge	

	merge 1:1 month using "$temp/ret_mid_vw.dta"
	drop _merge	
	
	merge 1:1 month using "$temp/ret_small_vw.dta"
	drop _merge		
...
*	keep if month >= 587  // 2009 onwards		
	...
	**** STATS WITH TRANSACTION COSTS
	tabstat ret_tc_full ret_tc_mega ret_tc_large ret_tc_mid ret_tc_small sp500 russel3000 if month_id > 1, stats(n mean sd min med max iqr sk k) c(s)
	
	**** STATS WITHOUT TRANSACTION COSTS
	tabstat ret_full ret_mega ret_large ret_mid ret_small sp500 russel3000 if month_id > 1, stats(n mean sd min med max iqr sk k) c(s)	
